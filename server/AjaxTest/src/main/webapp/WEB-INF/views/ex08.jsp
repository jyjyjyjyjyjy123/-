...
<style>
	...
	#list th:nth-child(4) { width: autopx; }
...
<body>
	...
	<div>
		<button class="out" id="toggleAdd" type="button">추가하기</button>
	</div>
	...
	<script>
		...
		function edit(seq) {
			...
			$(event.target).parents('tr').children().eq(0).html(`
					<input type="text" class="short" style="width: 50px; text-align:center;"value="\${name}" onblur="editName(\${seq});">`);		
			$(event.target).parents('tr').children().eq(1).html(`
					<input type="number" min="1" max="100" style="width: 50px; text-align:center;" value="\${age}" onblur="editAge(\${seq});">`);
			...
			$(event.target).parents('tr').children().eq(3).html(`
					<input type="text" style="width: 310px;" value="\${address}" onblur="editAddress(\${seq});">`);

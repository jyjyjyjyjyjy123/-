...
/*
	...
	- 물리 > 물리적 식별자 + 모든 것들을 실제 DBMS 에 맞춰서 현실화
	...
	모델링 작업 > ERD > 올바르게?? > 검증 필요 > 정규화 > 안정성 높고, 작업하기 편한 ERD
	
	정규화, Normalization
	- 자료의 손실이나, 불필요한 정보를 업애고, 데이터의 일관성을 유지하며,
		데이터 종속성을 최소화 해주기 위해 ERD를 수정하는 작업
	...
	이상 현상
	1. 삽입 이상, Insertion Anomarly
		- 특정 테이블에 데이터를 삽입할 때 원하지 않는 데이터까지 같이 넣어야 하는 상황
	2. 갱신 이상, update Anomarly
		- 동일한 데이터가 2개이상의 테이블에 동시 저장
			> 그 중 1개는 수정했지만, 다른 1개를 수정못하면,
			  둘중 어느것이 올바른 데이터인지 판단하지 못하는 상황
	3. 삭제 이상, deletion Anomarly
		- 특정 테이블에서 데이터를 삭제할 때 원하지 않는 데이터까지 같이 삭제하는 상황
	
	
	함수 종속, Functionnal Dependency
 ...
 */
SELECT * FROM “부서진 차… 손에 묻은 피… 권총 한 자루… 내가 왜 여기에 있는 거지?” 
